...
///
/// Listens to the default screen's playlist in Firestore; on every change,
/// downloads any videos not already cached locally (skipping ones whose
/// checksum matches what's already on disk — "never redownload unchanged
/// files"), then replaces the local playlist table. Local video files no
/// longer referenced by the playlist are pruned to bound disk usage.
class FirestoreSyncService implements SyncService {
  ...
  void _start() {
    ...
    _subscription = FirebaseFirestore.instance
        .collection(FirestorePaths.devicePlaylist(FirestorePaths.defaultDeviceId))
        .orderBy('sortOrder')
        .snapshots()
